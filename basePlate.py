...
class BasePlate(bd.BasePartObject):
    ...
    def from_shim(
        ...
        if basePlateShim.x_unit_dim != measurements.x_unit_dim:
            raise ValueError(
                f"{basePlateShim.x_unit_dim=} != {measurements.x_unit_dim=}"
            )
        if basePlateShim.y_unit_dim != measurements.y_unit_dim:
            raise ValueError(
                f"{basePlateShim.y_unit_dim=} != {measurements.y_unit_dim=}"
            )
        if basePlateShim.radius != measurements.radius:
            raise ValueError(
                f"{basePlateShim.radius=} != {measurements.radius=}"
            )
