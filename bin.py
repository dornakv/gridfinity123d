...
class Bin(bd.BasePartObject):
    def __init__(
        ...
        if height_units < 1: raise ValueError(f"{height_units=} must be at least 1")
    ...
    @property
    def height_units(self): return self._height_units
    @property
    def x_unit_dim(self): return self._measurements.x_unit_dim
    @property
    def y_unit_dim(self): return self._measurements.y_unit_dim
    @property
    ...
    @property
    def bin_gap(self): return self._measurements.bin_gap
    @property
    ...
    def _get_part(self):
        ...
        inner_floor_outline = common.GetRoundedRect(
            (self.x_units * self.x_unit_dim) - (2 * self.bin_gap) - (2 * self.wall_thickness),
            (self.y_units * self.y_unit_dim) - (2 * self.bin_gap) - (2 * self.wall_thickness),
            radius=self.radius
        )

        floor_plane = bd.Plane.XY.offset(self.retention_socket_measurements.height + self.floor_thickness)
        wall = None
        if self.height_units * self.height_unit_dim - self.retention_socket_measurements.height > constants.RESOLUTION:
            wall_face = bd.make_face(outer_floor_outline) - bd.make_face(inner_floor_outline)                        
